...
const contractAddress = '0x62D8B408F7B5f684e7F8e1Ee15f906187ce1844A';
...
export const endgame = async (amount,accountaddress) => {
	try {
		const accounts = await web3.eth.getAccounts();

		const winn = await casinoContract.methods.endGame().send({
			from: accounts[0],
		...
		console.log('Game ended successfully:', winn);
		return;
	} catch (error) {
