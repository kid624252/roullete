...
const GameWheel = ({ players, totalBet }) => {
    ...
    const totalShare = players.reduce((acc, player) => acc + player.share, 0);

    useEffect(() => {
        // Update chart data based on players' bets
        if (players.length > 0) {
            const data = players.map((player, index) => ({
                title: player.name.substring(0, 6) + '...',
                value: player.share,
            ...
            {players.map((player, index) => (
                <div
                    ...
                    style={{
                        ...
                        // transform: `rotate(${player.share / totalShare * 360}deg)`,
                        // anglePerSection
                        backgroundColor: player.color, // Set a unique color for each player
                    }}
                >
                    {/* <span className="player-info">
                        {player.name} <br /> {player.share} ETH
                    </span> */}
                </div>
            ))}
            <div className="game-wheel-center">
